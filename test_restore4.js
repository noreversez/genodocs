const fs = require('fs');

const text = fs.readFileSync('js/app.js', 'utf8');

const win1252ToByte = {
  0x20AC: 0x80, 0x0081: 0x81, 0x201A: 0x82, 0x0192: 0x83,
  0x201E: 0x84, 0x2026: 0x85, 0x2020: 0x86, 0x2021: 0x87,
  0x02C6: 0x88, 0x2030: 0x89, 0x0160: 0x8A, 0x2039: 0x8B,
  0x0152: 0x8C, 0x008D: 0x8D, 0x017D: 0x8E, 0x008F: 0x8F,
  0x0090: 0x90, 0x2018: 0x91, 0x2019: 0x92, 0x201C: 0x93,
  0x201D: 0x94, 0x2022: 0x95, 0x2013: 0x96, 0x2014: 0x97,
  0x02DC: 0x98, 0x2122: 0x99, 0x0161: 0x9A, 0x203A: 0x9B,
  0x0153: 0x9C, 0x009D: 0x9D, 0x017E: 0x9E, 0x0178: 0x9F
};

const bytes = Buffer.alloc(text.length * 4);
let byteCount = 0;

for (let i = 0; i < text.length; i++) {
  const code = text.charCodeAt(i);
  if (win1252ToByte[code] !== undefined) {
    bytes[byteCount++] = win1252ToByte[code];
  } else if (code <= 0xFF) {
    bytes[byteCount++] = code;
  } else {
    const buf = Buffer.from(text[i], 'utf8');
    for (let j = 0; j < buf.length; j++) {
        bytes[byteCount++] = buf[j];
    }
  }
}

const finalBuf = bytes.subarray(0, byteCount);
const restored = finalBuf.toString('utf8');

fs.writeFileSync('js/app.js.fixed2', restored, 'utf8');
const lines = restored.split('\n');
console.log(lines[321]);
